...
#' is however not fully impleme#' Targeted Record Swapping
#'
#' Applies targeted record swapping on micro data considering the identification
#' risk of each record as well the geographic topology.
#'
...
#' be swapped is made up of all records which do not fulfill the k-anonymity
...
#' that household with risk lower than threshold, but with still high enough      ###
#' risk may be swapped as well.                                                   ###
#' Risk threshold must be equal or bigger then 0.                                 ### 
#' @param k_anonymity integer defining the threshold of high risk households
#' (counts<k) for using k-anonymity rule. Default value is 3.
...
recordSwap.default <- function(data, hid, hierarchy, similar,
  ...
  if(nrow(risk)>0){
    ...
    # Check if risk values in each household are unique and if not then assign 
